...
def folder_new(request):
    ...
    # Any logged-in user may add a folder under an existing one. Creating a
    # TOP-LEVEL folder is admin-only (that's dashboard structure).
    if parent is None and not permissions.is_admin(request.user):
...
def folder_move(request, pk):
    ...
    if not permissions.is_admin(request.user):
        return _forbidden("Only administrators can move folders.")
...
def folder_edit(request, pk):
    folder = get_object_or_404(Folder, pk=pk)
    if not permissions.is_admin(request.user):
        return _forbidden("Only administrators can rename folders.")
...
def folder_delete(request, pk):
    folder = get_object_or_404(Folder, pk=pk)
    if not permissions.is_admin(request.user):
        return _forbidden("Only administrators can delete folders.")
