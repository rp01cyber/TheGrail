...
class Folder(TimeStamped):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = slugify(self.name) or "folder"
        base, n = self.slug, 2
        qs = Folder.objects.filter(parent=self.parent, slug=self.slug)
        if self.pk:
...
class Page(TimeStamped):
    ...
    def save(self, *args, **kwargs):
        ...
        # Guarantee the slug is unique within its folder so duplicate titles
        # don't hit the DB uniqueness constraint (which would 500).
        base, n = self.slug, 2
        qs = Page.objects.filter(folder=self.folder, slug=self.slug)
        if self.pk:
            qs = qs.exclude(pk=self.pk)
        while qs.exists():
            self.slug = f"{base}-{n}"
