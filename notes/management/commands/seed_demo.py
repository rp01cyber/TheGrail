...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        DEFAULT_TAGS = [
            "rdp", "smb", "windows", "linux", "active-directory", "lateral-movement",
            "privilege-escalation", "persistence", "credential-access", "web",
            "network", "enumeration", "exploitation", "post-exploitation",
            "cloud", "wireless",
        ]
        from notes.models import Tag as _Tag
        from django.utils.text import slugify as _slug
        for _n in DEFAULT_TAGS:
            _Tag.objects.get_or_create(slug=_slug(_n), defaults={"name": _n})
        self.stdout.write(self.style.SUCCESS(f"Seeded {len(DEFAULT_TAGS)} default tags"))
