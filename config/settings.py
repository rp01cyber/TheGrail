...
# --- Authentication backends (axes must wrap the default) -------------------
AUTHENTICATION_BACKENDS = [
    "axes.backends.AxesStandaloneBackend",          # checks lockout first
    "django.contrib.auth.backends.ModelBackend",    # normal username/password
]

# --- Login rate-limiting / account lockout (django-axes) --------------------
# Lock an account after 5 failed attempts. No automatic timed unlock — an
# administrator must clear it (Users page -> Unlock, or the axes_reset command).
AXES_FAILURE_LIMIT = int(env("PN_LOCKOUT_LIMIT", "5"))
AXES_COOLOFF_TIME = None                    # None = stay locked until an admin resets
AXES_LOCKOUT_PARAMETERS = ["username"]      # lock the account, not just an IP
AXES_RESET_ON_SUCCESS = True                # a good login clears that user's counter
AXES_LOCKOUT_TEMPLATE = "notes/lockout.html"
AXES_LOCKOUT_HTTP_STATUS = 429
AXES_VERBOSE = True
# Behind Caddy the real client IP is in X-Forwarded-For.
AXES_IPWARE_PROXY_COUNT = 1
AXES_IPWARE_META_PRECEDENCE_ORDER = ["HTTP_X_FORWARDED_FOR", "REMOTE_ADDR"]
# We deliberately lock by account (username), per requirement, and require an
# admin to unlock. axes warns this doesn't also throttle by IP; that's an
# accepted trade-off. See notes in README about optionally adding IP limiting.
...
# Sessions expire 1 day after login (override with PN_SESSION_AGE, in seconds).
# This is a fixed window from login, not rolling; set SESSION_SAVE_EVERY_REQUEST
# to True below if you'd rather the clock reset on each request (activity-based).
SESSION_COOKIE_AGE = int(env("PN_SESSION_AGE", "86400"))  # 86400s = 24 hours
SESSION_SAVE_EVERY_REQUEST = env_bool("PN_SESSION_ROLLING", False)
